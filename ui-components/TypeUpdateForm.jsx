...
export default function TypeUpdateForm(props) {
  const {
    id: idProp,
    type: typeModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = typeRecord
      ? { ...initialValues, ...typeRecord }
      : initialValues;
    setName(cleanValues.name);
    setActive(cleanValues.active);
    setErrors({});
  };
  const [typeRecord, setTypeRecord] = React.useState(typeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getType.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getType
        : typeModelProp;
    ...
    queryData();
  }, [idProp, typeModelProp]);
  React.useEffect(resetStateValues, [typeRecord]);
  const validations = {
    name: [{ type: "Required" }],
    active: [],
  };
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateType.replaceAll("__typename", ""),
            variables: {
              input: {
                id: typeRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "TypeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || typeModelProp)}
